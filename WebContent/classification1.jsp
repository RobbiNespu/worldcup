...
<%
  ...
  int points3 = c.winners; 
        ...
        tooltipHTML = tooltipHTML.replaceFirst("last6", "<B>" + last6 + "p</B> ("+last6s+ " - " + last6w + " - " + (6-last6s-last6w) + ")");
		  ...
		  if (user.equals(gameuser))	out.write("<tr titlex=\""+tooltip+"\" class=ROWUSER onmouseover=\"tooltip.show('"+tooltipHTML+"', 300);return;ShowDiv(top.event,'"+tooltipHTML+"')\"  >");
			else out.write("<tr titlex=\""+tooltip+"\" class=ROW"+((counter%2==1)?"ODD":"EVEN")+" onmouseover=\"tooltip.show('"+tooltipHTML+"', 300);return;ShowDiv(event,'"+tooltipHTML+"')\" >");
			...
			out.write("<td class=FCELL><B>"+(gameuser.length()>9?gameuser.substring(0,9):gameuser)+"</B></td>");
